...
Chain.prototype.verify = co(function* verify(block, prev, flags) {
  ...
  if (state.hasExtension()) {
    ...
    if (commit) {
      hash = block.createCommitmentHash();

      if (!hash) {
        throw new VerifyError(block,
          'invalid',
          'bad-witness-merkle-malleated',
          100,
          true);
      }

      if (!util.equal(commit, hash)) {
        throw new VerifyError(block,
          'invalid',
  ...
  // an extension cannot contain one.
  if (!commit) {
    ...
    // Resolution is only required if:
    // - The block has a commitment hash.
    // AND:
    // - A previous resolution has been mined.
    // OR:
    // - The extension block has entering inputs.
    //
    // In other words, the first genesis
    // resolution is triggered by entering
    // outputs.
    if (prev.resHash !== encoding.NULL_HASH || block.hasEntrance()) {
      if (block.txs.length < 2) {
        throw new VerifyError(block,
          'invalid',
          'expected-resolution',
          100);
...
Chain.prototype.getDeployments = co(function* getDeployments(ts, prev) {
  ...
  // Extension blocks are now usable.
  active = yield this.isActive(prev, deployments.extblk);
  if (active)
...
Chain.prototype.setDeploymentState = function setDeploymentState(state) {
  ...
  if (!this.state.hasExtension() && state.hasExtension())
    this.logger.warning('Extension blocks have been activated.');
...
Chain.prototype.verifyInputs = co(function* verifyInputs(block, prev, state) {
  ...
  view.resHash = prev.resHash;
  view.resValue = prev.resValue;

  // Skip everything in SPV.
  if (this.options.spv)
    return view;

  // Verify the canonical block first.
  yield this.verifyCanonical(block, prev, view, state);

  // Verify the extension block and resolution.
  if (state.hasResolution()) {
    yield this.verifyExtension(block, prev, view, state);

    // Mark the new resolution.
    res = block.txs[block.txs.length - 1];
    view.resHash = res.hash('hex');
    view.resValue = res.outputs[0].value;
  }
...
Chain.prototype.verifyCanonical = co(function* verifyCanonical(block, prev, view, state) {
  ...
  var reward = 0;
  var jobs = [];
  var i, tx, valid, fee, coins;

  // Check all transactions
  for (i = 0; i < block.txs.length; i++) {
    tx = block.txs[i];

    if (i === block.txs.length - 1)
      resolution = state.hasResolution();
    ...
    // using checkpoints.
    if (historical) {
      coins = view.addTX(tx, height, false);
      coins.resolution = resolution;
      continue;
    }
    ...
    sigops += tx.getSigops(view, state.flags);
    ...
    if (i > 0) {
      fee = tx.checkContext(view, height, resolution, ret);
      ...
      // Do not allow cross-chain spends.
      if (state.hasExtension()) {
        if (!resolution && !tx.isCanonical(view)) {
          throw new VerifyError(block,
            'invalid',
            'bad-txns-inputs-wrongchain',
            100);
        }
      }
...
/**
 * Verify extension inputs and resolution transaction.
 * @method
 * @private
 * @param {Block} block
 * @param {ChainEntry} prev
...
Chain.prototype.verifyExtension = co(function* verifyExtension(block, prev, view, state) {
  var ret = new VerifyResult();
  var height = prev.height + 1;
  var historical = prev.isHistorical();
  var res = block.txs[block.txs.length - 1];
  var value = 0;
  var cost = 0;
  var fees = 0;
  var jobs = [];
  var i, input, tx, valid, fee;

  assert(state.hasExtension());
  assert(state.hasResolution());

  // Verify the resolution transaction
  // and calculate expected value.
  value = this.verifyResolution(block, prev);

  // Unspend the first input if we're
  // the genesis resolution transaction.
  if (prev.resHash === encoding.NULL_HASH) {
    input = res.inputs[0];
    view.enter(input);
  }

  // Unspend the coins that entered
  // the extension block.
  for (i = 1; i < res.inputs.length; i++) {
    input = res.inputs[i];
    view.enter(input);
  }

  // Check all extension transactions.
  for (i = 0; i < block.ext.length; i++) {
    tx = block.ext[i];

    // Do not allow cross-chain spends.
    if (!tx.isExtension(view)) {
      throw new VerifyError(block,
        'invalid',
        'bad-txns-inputs-wrongchain',
        100);
    ...
    // Calculate inputs and outputs cost.
    cost += tx.getExtensionCost(view);

    if (cost > consensus.MAX_EXTENSION_COST) {
      throw new VerifyError(block,
        'invalid',
        'bad-blk-sigops',
        100);
    ...
    // Contextual sanity checks.
    fee = tx.checkContext(view, height, false, ret);
    ...
    // Push onto verification queue.
    jobs.push(tx.verifyAsync(view, state.extFlags));
  ...
  // ((prev-value + entering-value - exiting-value) - ext-block-fees)
  if (res.outputs[0].value !== value - fees) {
    throw new VerifyError(block,
      'invalid',
      'bad-resolution-amount',
      100);
  }
 ...
 * @param {Block} block
 * @param {ChainEntry} prev
 * @param {Amount} extFees
 * @returns {Boolean}
 * @throws {VerifyError}
 */

Chain.prototype.verifyResolution = function verifyResolution(block, prev) {
  var res = block.txs[block.txs.length - 1];
  var value = prev.resValue;
  var index = 0;
  var i, j, tx, output;

  // Require version to be uint32_max.
  if (res.version !== 0xffffffff) {
    throw new VerifyError(block,
      'invalid',
      'bad-resolution-version',
      100);
  }

  // Genesis resolution transaction
  // does not need a previous resolution.
  if (prev.resHash !== encoding.NULL_HASH) {
    if (res.inputs[0].prevout.hash !== prev.resHash) {
      throw new VerifyError(block,
        'invalid',
        'bad-resolution-outpoint-hash',
        100);
    }

    if (res.inputs[0].prevout.index !== 0) {
      throw new VerifyError(block,
        'invalid',
        'bad-resolution-outpoint-index',
        100);
    }

    if (res.inputs[0].script.raw.length !== 0) {
      throw new VerifyError(block,
        'invalid',
        'bad-resolution-input-script',
        100);
    }

    if (res.inputs[0].sequence !== 0xffffffff) {
      throw new VerifyError(block,
        'invalid',
        'bad-resolution-input-sequence',
        100);
    }

    assert(res.inputs[0].witness.items.length === 0);
  ...
  for (i = 0; i < block.txs.length - 1; i++) {
    ...
    for (j = 0; j < tx.outputs.length; j++) {
      ...
      // Coinbases cannot directly enter
      ...
      if (index >= res.inputs.length) {
        throw new VerifyError(block,
          'invalid',
          'bad-resolution-input-len',
          100);
      }

      if (res.inputs[index].prevout.hash !== tx.hash('hex')) {
        throw new VerifyError(block,
          'invalid',
          'bad-resolution-outpoint-hash',
          100);
      ...
      if (res.inputs[index].prevout.index !== j) {
        throw new VerifyError(block,
          'invalid',
          'bad-resolution-outpoint-index',
          100);
      ...
      if (res.inputs[index].script.raw.length !== 0) {
        throw new VerifyError(block,
          'invalid',
          'bad-resolution-input-script',
          100);
      ...
      if (res.inputs[index].sequence !== 0xffffffff) {
        throw new VerifyError(block,
          'invalid',
          'bad-resolution-input-sequence',
          100);
      ...
      assert(res.inputs[index].witness.items.length === 0);
  ...
  // First output must be OP_TRUE.
  if (!res.outputs[0].script.isResolution()) {
    throw new VerifyError(block,
      'invalid',
      'bad-resolution-output-script',
      100);
  }
  ...
  for (i = 0; i < block.ext.length; i++) {
    ...
    // All exiting outputs should be duplicated
    ...
    for (j = 0; j < tx.outputs.length; j++) {
      ...
      if (index >= res.outputs.length) {
        throw new VerifyError(block,
          'invalid',
          'bad-resolution-output-len',
          100);
      }

      if (!util.equal(res.outputs[index].script.raw, output.script.raw)) {
        throw new VerifyError(block,
          'invalid',
          'bad-resolution-exit-script',
          100);
      }

      if (res.outputs[index].value !== output.value) {
        throw new VerifyError(block,
          'invalid',
          'bad-resolution-exit-amount',
          100);
      }
  ...
  // Locktime check to make sure people
  // aren't doing anything weird.
  if (res.locktime !== 0) {
    throw new VerifyError(block,
      'invalid',
      'bad-resolution-locktime',
      100);
  }
...
Chain.prototype.reconnect = co(function* reconnect(entry) {
  ...
  entry.resHash = result.view.resHash;
  entry.resValue = result.view.resValue;
 ...
 * @example
 * yield chain.isActive(tip, deployments.extblk);
 * @method
 ...
 * @method
 * @example
 * yield chain.getState(tip, deployments.extblk);
 * @see https://github.com/bitcoin/bips/blob/master/bip-0009.mediawiki
 * @param {ChainEntry} prev - Previous chain entry.
 * @param {String} id - Deployment id.
 * @returns {Promise} - Returns Number.
...
function DeploymentState() {
  ...
  this.extFlags = Script.flags.MANDATORY_EXTENSION_FLAGS;
...
DeploymentState.prototype.hasExtension = function hasExtension() {
...
DeploymentState.prototype.hasResolution = function hasResolution() {
