...
Block.prototype.fromOptions = function fromOptions(options) {
  ...
  if (options.ext) {
    ...
    for (i = 0; i < options.ext.length; i++)
      this.ext.push(options.ext[i]);
  }
};
...
/**
 * Serialize the block. Include extension if present.
 * @returns {Buffer}
 */
...
/**
 * Serialize the block, do not include extension.
 * @returns {Buffer}
 */
...
/**
 * Calculate real size and size of the extension bytes.
 * @returns {Object} Contains `total` and `extension`.
 */
...
/**
 * Get base block size (without extension).
 * @returns {Number} size
 */
...
/**
 * Test whether the block contains an extension.
 * @returns {Boolean}
 */
...
/**
 * Calculate commitment hash (the root
 * of the wtxid merkle tree).
 * @param {String?} enc - Encoding, can be `'hex'` or null.
 * @returns {Hash}
 */
...
Block.prototype.verifyBody = function verifyBody(ret) {
  ...
  if (this.ext.length > consensus.MAX_EXTENSION_SIZE
      || this.getExtensionSize() > consensus.MAX_EXTENSION_SIZE) {
    ret.reason = 'bad-extension-length';
    ret.score = 100;
    return false;
  }
  ...
  for (i = 0; i < this.ext.length; i++) {
    ...
    // Input scripts cannot be present
    // in the extension block.
    if (tx.hasScript()) {
      ret.reason = 'unexpected-scriptsig';
      ret.score = 100;
      return false;
    }

    // uint32_max is resolution-only.
    if (tx.version === 0xffffffff) {
      ret.reason = 'bad-txns-version';
      ret.score = 100;
      return false;
    }

    // Outputs must be programs, p2pkh, or p2sh.
    if (!tx.verifyExits()) {
      ret.reason = 'bad-txns-outputs-badexit';
      ret.score = 100;
      return false;
    }

    // Outputs must not be dust.
    if (tx.hasDust()) {
      ret.reason = 'dust';
      ret.score = 100;
      return false;
...
/**
 * Serialze block without extension data.
 * @private
...
/**
 * Serialze block with extension data.
 * @private
 * @param {BufferWriter?} writer
 * @returns {Buffer}
 */
...
/**
 * Serialze block with or without extension data.
 * @private
 * @param {BufferWriter?} writer
 * @returns {Buffer}
 */
...
/**
 * Get block size without extension.
 * @returns {RawBlock}
 */
