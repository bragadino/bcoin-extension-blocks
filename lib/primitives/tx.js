...
/**
 * Calculate sigops+outputs cost.
 * @param {CoinView} view
 * @returns {Number} ext. tx cost
 */

TX.prototype.getWeightedSize = function getWeightedSize(extended) {
  var size = this.getSize();

  if (extended) {
    size += this.getExitSize() * 2;
    return size;
  }

  size += this.getEnterSize();
...
/**
 * Calculate total size of entering outputs.
 * @returns {Number} size
 */

TX.prototype.getEnterSize = function getEnterSize() {
  var total = 0;
  var i, output;
...
/**
 * Calculate total size of exiting outputs.
 * @returns {Number} size
 */

TX.prototype.getExitSize = function getExitSize() {
  var total = 0;
  var i, output;
...
/**
 * Calculate total legacy sigops of exiting outputs.
 * @returns {Number} sigops
 */

TX.prototype.getExitSigops = function getExitSigops() {
  var total = 0;
  var i, output;
...
/**
 * Get the chain the transaction is redeeming from.
 * @param {CoinView} view
 * @param {Number} flags
 * @returns {Number}
 */

TX.prototype.getChain = function getChain(view) {
  var chain = -1;
  var i, input;
...
/**
 * Verify extension outputs (consensus).
 * @returns {Boolean}
 */

TX.prototype.verifyExits = function verifyExits() {
  var i, output;
  ...
  for (i = 0; i < this.outputs.length; i++) {
    ...
    // Exit (p2pkh - canonical encoding)
    if (output.script.isPubkeyhash(true))
      continue;

    // Exit (p2sh)
    if (output.script.isScripthash())
      continue;

    // Non-p2pkh and non-p2sh scripts are
    // invalid in the extension block.
    return false;
  }
...
/**
 * Test whether the transaction has a non-empty scriptsig.
 * @returns {Boolean}
 */

TX.prototype.hasScript = function hasScript() {
  var i, input;
...
/**
 * Test whether the transaction is a resolution.
 * @returns {Boolean}
 */

TX.prototype.isResolution = function isResolution() {
  return this.version === 0xffffffff;
};
...
TX.prototype.getSigops = function getSigops(view, flags) {
  var total = this.getLegacySigops();
  ...
  if (flags & Script.flags.VERIFY_P2SH)
    total += this.getScripthashSigops(view);
...
TX.prototype.getExtensionCost = function getExtensionCost(view) {
  var cost = 0;
  ...
  for (i = 0; i < this.inputs.length; i++) {
    ...
    cost += coin.script.getInputCost(input.witness);
  }

  for (i = 0; i < this.outputs.length; i++) {
    output = this.outputs[i];
    cost += output.script.getOutputCost();
  }
...
/**
 * Test whether the transaction has all canonical inputs.
 * @param {CoinView} view
 * @returns {Boolean}
 */

TX.prototype.isCanonical = function isCanonical(view) {
  var i, input;
...
/**
 * Test whether the transaction has all program inputs.
 * @param {CoinView} view
 * @returns {Boolean}
 */

TX.prototype.isExtension = function isExtension(view) {
  var i, input;
...
TX.prototype.checkInputs = function checkInputs(view, height, resolution, ret) {
  return this.checkContext(view, height, resolution, ret) !== -1;
};
...
TX.prototype.checkContext = function checkContext(view, height, resolution, ret) {
  var total = 0;
  var value = 0;
  var i, input, coins, coin, fee;
  ...
  for (i = 0; i < this.inputs.length; i++) {
    ...
    // The resolution pushdata is NOT
    // spendable by anybody but a miner.
    if (!resolution && coins.resolution && input.prevout.index === 0) {
      ret.reason = 'bad-txns-inputs-resolution-redemption';
      // Set score to zero to avoid
      // banning every non-upgraded node.
      ret.score = 0;
      return -1;
    }
...
/**
 * Check dust threshold on program outputs.
 * @returns {Boolean}
 */

TX.prototype.verifyDust = function verifyDust() {
  var i, output;

  for (i = 0; i < this.outputs.length; i++) {
    output = this.outputs[i];

    if (output.value < 500 && output.script.isProgram())
      return false;
  }
...
TX.prototype.writeWitness = function writeWitness(bw) {
  ...
  for (i = 0; i < this.inputs.length; i++) {
    input = this.inputs[i];
    // assert(input.script.getSize() === 0,
    //   'Non-zero script in witness serialization.');
    input.toWriter(bw);
  }
...
TX.prototype.getNormalSize = function getNormalSize() {
  var total = 0;
  var i, input, output;
...
TX.prototype.getWitnessSize = function getWitnessSize() {
  var total = 0;
  var i, input, output;
  ...
  for (i = 0; i < this.inputs.length; i++) {
    input = this.inputs[i];
    total += input.getSize();
    total += input.witness.getVarSize();
  }
