...
Mempool.prototype.addBlock = co(function* addBlock(block, txs, ext) {
  var unlock = yield this.locker.lock();
  try {
    return yield this._addBlock(block, txs, ext);
  } finally {
    unlock();
  }
});
...
Mempool.prototype._addBlock = co(function* addBlock(block, txs, ext) {
...
Mempool.prototype.removeBlock = co(function* removeBlock(block, txs, ext) {
  var unlock = yield this.locker.lock();
  try {
    return yield this._removeBlock(block, txs, ext);
  } finally {
    unlock();
  }
});
...
Mempool.prototype._removeBlock = co(function* removeBlock(block, txs, ext) {
  var total = 0;
  var i, tx, hash;
  ...
  for (i = 1; i < txs.length; i++) {
    ...
    try {
      ...
      if (e.type === 'VerifyError') {
        this.logger.warning('Reorganized transaction failed verification.');
...
/**
 * Check to see if a coin is exiting. These coins
 * are not spendable until the next block.
 * @param {Hash} hash
 * @param {Number} index
 * @returns {Boolean}
 */

Mempool.prototype.isExit = function isExit(hash, index) {
  var key = Outpoint.toKey(hash, index);
  return this.exits[key] != null;
};
...
Mempool.prototype.insertTX = co(function* insertTX(tx) {
  ...
  if (!this.chain.state.hasExtension() && !this.options.prematureWitness) {
    if (tx.hasWitness()) {
      throw new VerifyError(tx,
        'nonstandard',
        'no-witness-yet',
  ...
  // Make sure we're not allowing
  // an early spend of an exit.
  if (this.isPrematureSpend(tx)) {
    throw new VerifyError(tx,
      'invalid',
      'bad-txns-inputs-earlyspend',
      0);
  }
  ...
  // Are we on the ext. block sidechain?
  chain = tx.getChain(view);

  // Disallow cross-chain spending.
  if (chain === -1) {
    throw new VerifyError(tx,
      'invalid',
  ...
  // cannot have a witness vector.
  if (chain === 0 && tx.hasWitness()) {
    throw new VerifyError(tx,
      'invalid',
      'unexpected-witness',
      100,
  ...
  // cannot have a scriptsig.
  if (chain === 1 && tx.hasScript()) {
    throw new VerifyError(tx,
      'invalid',
      'unexpected-scriptsig',
      100);
  }

  if (chain === 1 && !tx.verifyExits()) {
    throw new VerifyError(tx,
      'invalid',
      'bad-txns-outputs-exits',
      100);
  }
...
Mempool.prototype.verify = co(function* verify(entry, view) {
  ...
  if (this.options.requireStandard) {
    ...
    if (this.chain.state.hasExtension()) {
      if (entry.extended && !tx.hasStandardWitness(view)) {
        throw new VerifyError(tx,
          'nonstandard',
  ...
  if (entry.cost > policy.MAX_TX_COST) {
    throw new VerifyError(tx,
      'nonstandard',
      'bad-txns-too-much-cost',
      0);
  }
  ...
  if (!tx.checkInputs(view, height, false, ret))
...
/**
 * Test all of a transactions outpoints
 * to check for an early spend of an output.
 * @param {TX} tx
 * @returns {Promise} - Returns Boolean.
 */

Mempool.prototype.isPrematureSpend = function isPrematureSpend(tx) {
  var i, input, prevout;
  ...
  for (i = 0; i < tx.inputs.length; i++) {
    ...
    if (this.isExit(prevout.hash, prevout.index))
      return true;
  }
...
Mempool.prototype.getCoinView = co(function* getCoinView(tx) {
  ...
  for (i = 0; i < prevout.length; i++) {
    ...
    if (entry) {
      view.addTX(entry.tx, -1, entry.extended);
      continue;
    }
...
Mempool.prototype.trackEntry = function trackEntry(entry, view) {
  ...
  // Exits cannot be directly spent until
  // the next block is mined. At which
  // point they must be redeemed from the
